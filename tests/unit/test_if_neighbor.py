"""Unit tests for opnsense.managers.if_neighbor.IfNeighborManager.
...
Tests cover all BaseManager endpoints:
    ...
    - field validation: required ipaddress, etheraddr
...
class TestEnsurePresent:
    ...
    async def test_create_new_neighbor(self, mock_client: AsyncMock) -> None:
        mock_client.search.return_value = []
        mock_client.create.return_value = "uuid-new"
        mock_client.reconfigure.return_value = {"status": "ok"}

        mgr = IfNeighborManager(mock_client)
        result = await mgr.ensure(state="present", params=NEIGHBOR_PARAMS)

        assert result.changed is True
        assert result.action == "created"
        assert result.uuid == "uuid-new"
        mock_client.create.assert_awaited_once_with(
            "interfaces/neighbor_settings/addItem", "neighbor", NEIGHBOR_PARAMS
        )
        mock_client.reconfigure.assert_awaited_once_with(
            "interfaces/neighbor_settings/reconfigure", timeout=None
        )

    async def test_noop_when_no_drift(self, mock_client: AsyncMock) -> None:
        mock_client.search.return_value = [{"uuid": "uuid-1", **NEIGHBOR_PARAMS}]
    ...
    async def test_update_when_drift_detected(self, mock_client: AsyncMock) -> None:
        mock_client.search.return_value = [{"uuid": "uuid-1", **NEIGHBOR_PARAMS}]
        mock_client.get.return_value = {"neighbor": {"uuid": "uuid-1", **NEIGHBOR_PARAMS}}
...
class TestEnsureAbsent:
    ...
    async def test_delete_existing_neighbor(self, mock_client: AsyncMock) -> None:
        mock_client.search.return_value = [{"uuid": "uuid-1", **NEIGHBOR_PARAMS}]
        mock_client.get.return_value = {"neighbor": {"uuid": "uuid-1", **NEIGHBOR_PARAMS}}
        ...
        result = await mgr.ensure(
            state="absent",
            params={"ipaddress": "10.0.0.1", "etheraddr": "00:11:22:33:44:55"},
        )
    ...
    async def test_noop_when_already_absent(self, mock_client: AsyncMock) -> None:
        ...
        mgr = IfNeighborManager(mock_client)
        result = await mgr.ensure(state="absent", params={"ipaddress": "10.0.0.99"})
...
class TestCheckMode:
    ...
    async def test_check_mode_create_no_api_call(self, mock_client: AsyncMock) -> None:
        ...
        mgr = IfNeighborManager(mock_client)
        result = await mgr.ensure(state="present", params=NEIGHBOR_PARAMS, check_mode=True)
...
class TestErrorHandling:
    ...
    async def test_create_failure_logs_error_and_reraises(
        ...
        mock_client.create.side_effect = OpnsenseValidationError(
            message="invalid ip",
            endpoint="interfaces/neighbor_settings/addItem",
        )
        ...
        with (
            ...
            await mgr.ensure(state="present", params=NEIGHBOR_PARAMS)
    ...
    async def test_invalid_state_raises_value_error(self, mock_client: AsyncMock) -> None:
        ...
        with pytest.raises(ValueError, match="Invalid state"):
            await mgr.ensure(state="running", params={"ipaddress": "10.0.0.1"})
    ...
    async def test_error_preserves_exception_type(self, mock_client: AsyncMock) -> None:
        ...
        mock_client.create.side_effect = OpnsenseValidationError(
            message="bad input",
            endpoint="interfaces/neighbor_settings/addItem",
            validations={"neighbor.ipaddress": "required"},
        )
        ...
        assert exc_info.value.validations == {"neighbor.ipaddress": "required"}
...
class TestFieldValidation:
    ...
    async def test_missing_required_ipaddress(self, mock_client: AsyncMock) -> None:
        mgr = IfNeighborManager(mock_client)
        with pytest.raises(FieldValidationError):
            await mgr.ensure("present", params={"etheraddr": "00:11:22:33:44:55"})
        mock_client.create.assert_not_awaited()

    async def test_missing_required_etheraddr(self, mock_client: AsyncMock) -> None:
        mgr = IfNeighborManager(mock_client)
        with pytest.raises(FieldValidationError):
            await mgr.ensure("present", params={"ipaddress": "10.0.0.1"})
        mock_client.create.assert_not_awaited()
...
class TestAmbiguousMatch:
    ...
    async def test_ambiguous_match_raises(self, mock_client: AsyncMock) -> None:
        ...
        with pytest.raises(AmbiguousMatchError) as exc_info:
            await mgr.ensure("present", params=NEIGHBOR_PARAMS)
