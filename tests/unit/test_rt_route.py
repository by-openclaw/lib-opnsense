...
class TestEnsurePresent:
    async def test_create(self, mock_client: AsyncMock) -> None:
        ...
        result = await mgr.ensure("present", {"descr": "test route"})

        assert result.changed is True
        assert result.action == "created"
        mock_client.reconfigure.assert_awaited_once_with("routes/routes/reconfigure")

    async def test_noop(self, mock_client: AsyncMock) -> None:
        mock_client.search.return_value = [
            {"uuid": "uuid-1", "descr": "test route"},
        ]
...
class TestEnsureAbsent:
    async def test_delete(self, mock_client: AsyncMock) -> None:
        mock_client.search.return_value = [{"uuid": "uuid-1", "descr": "test route"}]
        mock_client.get.return_value = {"route": {"uuid": "uuid-1", "descr": "test route"}}
        mock_client.delete.return_value = {"result": "deleted"}
        mock_client.reconfigure.return_value = {"status": "ok"}

        mgr = RtRouteManager(mock_client)
        result = await mgr.ensure("absent", {"descr": "test route"})
...
class TestErrorHandling:
    async def test_create_failure_reraises(
        ...
        with (
            ...
            await mgr.ensure("present", {"descr": "test route"})
    ...
    async def test_invalid_state(self, mock_client: AsyncMock) -> None:
        ...
        with pytest.raises(ValueError, match="Invalid state"):
            await mgr.ensure("running", {"descr": "test route"})
