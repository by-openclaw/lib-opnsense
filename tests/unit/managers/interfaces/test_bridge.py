"""Unit tests for opnsense.managers.if_bridge.IfBridgeManager.
...
Tests cover all BaseManager endpoints:
    ...
    - field validation: required descr
"""
...
class TestEnsurePresent:
    ...
    async def test_create_new_bridge(self, mock_client: AsyncMock) -> None:
        """ensure present when bridge does not exist -> create + reconfigure."""
        mock_client.search.return_value = []
        mock_client.create.return_value = "uuid-new"
        mock_client.reconfigure.return_value = {"status": "ok"}

        mgr = IfBridgeManager(mock_client)
        result = await mgr.ensure(state="present", params=BRIDGE_PARAMS)

        assert result.changed is True
        assert result.action == "created"
        assert result.uuid == "uuid-new"
        mock_client.create.assert_awaited_once_with(
            "interfaces/bridge_settings/addItem", "bridge", BRIDGE_PARAMS
        )
        mock_client.reconfigure.assert_awaited_once_with(
            "interfaces/bridge_settings/reconfigure", timeout=None
        )

    async def test_noop_when_no_drift(self, mock_client: AsyncMock) -> None:
        """ensure present when bridge exists with matching params -> noop."""
        mock_client.search.return_value = [
            {"uuid": "uuid-1", **BRIDGE_PARAMS},
        ]
    ...
    async def test_update_when_drift_detected(self, mock_client: AsyncMock) -> None:
        """ensure present when members changed -> update + reconfigure."""
        mock_client.search.return_value = [
            {"uuid": "uuid-1", **BRIDGE_PARAMS},
        ]
        mock_client.get.return_value = {
            "bridge": {"uuid": "uuid-1", **BRIDGE_PARAMS},
        }
        ...
        result = await mgr.ensure(
            state="present",
            params={"descr": "LAN Bridge", "members": "vtnet1,vtnet2,vtnet3"},
        )
...
class TestEnsureAbsent:
    ...
    async def test_delete_existing_bridge(self, mock_client: AsyncMock) -> None:
        """ensure absent when bridge exists -> delete + reconfigure."""
        mock_client.search.return_value = [{"uuid": "uuid-1", **BRIDGE_PARAMS}]
        mock_client.get.return_value = {"bridge": {"uuid": "uuid-1", **BRIDGE_PARAMS}}
        mock_client.delete.return_value = {"result": "deleted"}
        mock_client.reconfigure.return_value = {"status": "ok"}

        mgr = IfBridgeManager(mock_client)
        result = await mgr.ensure(state="absent", params={"descr": "LAN Bridge"})
    ...
    async def test_noop_when_already_absent(self, mock_client: AsyncMock) -> None:
        """ensure absent when bridge does not exist -> noop."""
        mock_client.search.return_value = []
...
class TestCheckMode:
    ...
    async def test_check_mode_create_no_api_call(self, mock_client: AsyncMock) -> None:
        ...
        mgr = IfBridgeManager(mock_client)
        result = await mgr.ensure(state="present", params=BRIDGE_PARAMS, check_mode=True)
    ...
    async def test_check_mode_delete_no_api_call(self, mock_client: AsyncMock) -> None:
        ...
        mgr = IfBridgeManager(mock_client)
        result = await mgr.ensure(state="absent", params={"descr": "LAN Bridge"}, check_mode=True)
...
class TestErrorHandling:
    ...
    async def test_create_failure_logs_error_and_reraises(
        ...
        mock_client.create.side_effect = OpnsenseValidationError(
            message="descr required", endpoint="interfaces/bridge_settings/addItem"
        )
        ...
        with (
            ...
            await mgr.ensure(state="present", params=BRIDGE_PARAMS)
    ...
    async def test_delete_failure_logs_error_and_reraises(
        ...
        with (
            ...
            await mgr.ensure(state="absent", params={"descr": "LAN Bridge"})
    ...
    async def test_error_preserves_exception_type(self, mock_client: AsyncMock) -> None:
        ...
        mock_client.create.side_effect = OpnsenseValidationError(
            message="bad input",
            endpoint="interfaces/bridge_settings/addItem",
            validations={"bridge.descr": "required"},
        )
        ...
        assert exc_info.value.validations == {"bridge.descr": "required"}
...
class TestFieldValidation:
    ...
    async def test_missing_required_descr_raises_before_api_call(
        ...
        with pytest.raises(FieldValidationError):
            await mgr.ensure("present", params={"members": "vtnet1"})
        mock_client.create.assert_not_awaited()

    async def test_empty_descr_raises_before_api_call(self, mock_client: AsyncMock) -> None:
        mgr = IfBridgeManager(mock_client)
        with pytest.raises(FieldValidationError):
            await mgr.ensure("present", params={"descr": ""})
        mock_client.create.assert_not_awaited()
