"""Unit tests for opnsense.managers.if_lagg.IfLaggManager.
...
Tests cover all BaseManager endpoints:
    ...
    - field validation: required descr, enum proto
"""
...
class TestEnsurePresent:
    ...
    async def test_create_new_lagg(self, mock_client: AsyncMock) -> None:
        mock_client.search.return_value = []
        mock_client.create.return_value = "uuid-new"
        mock_client.reconfigure.return_value = {"status": "ok"}

        mgr = IfLaggManager(mock_client)
        result = await mgr.ensure(state="present", params=LAGG_PARAMS)

        assert result.changed is True
        assert result.action == "created"
        assert result.uuid == "uuid-new"
        mock_client.create.assert_awaited_once_with(
            "interfaces/lagg_settings/addItem", "lagg", LAGG_PARAMS
        )
        mock_client.reconfigure.assert_awaited_once_with(
            "interfaces/lagg_settings/reconfigure", timeout=None
        )

    async def test_noop_when_no_drift(self, mock_client: AsyncMock) -> None:
        mock_client.search.return_value = [{"uuid": "uuid-1", **LAGG_PARAMS}]
    ...
    async def test_update_when_drift_detected(self, mock_client: AsyncMock) -> None:
        mock_client.search.return_value = [{"uuid": "uuid-1", **LAGG_PARAMS}]
        mock_client.get.return_value = {"lagg": {"uuid": "uuid-1", **LAGG_PARAMS}}
        ...
        result = await mgr.ensure(
            state="present",
            params={"descr": "Bond0", "members": "vtnet1,vtnet2,vtnet3", "proto": "lacp"},
        )
...
class TestEnsureAbsent:
    ...
    async def test_delete_existing_lagg(self, mock_client: AsyncMock) -> None:
        mock_client.search.return_value = [{"uuid": "uuid-1", **LAGG_PARAMS}]
        mock_client.get.return_value = {"lagg": {"uuid": "uuid-1", **LAGG_PARAMS}}
        ...
        mgr = IfLaggManager(mock_client)
        result = await mgr.ensure(state="absent", params={"descr": "Bond0"})
...
class TestCheckMode:
    ...
    async def test_check_mode_create_no_api_call(self, mock_client: AsyncMock) -> None:
        ...
        mgr = IfLaggManager(mock_client)
        result = await mgr.ensure(state="present", params=LAGG_PARAMS, check_mode=True)
    ...
    async def test_check_mode_delete_no_api_call(self, mock_client: AsyncMock) -> None:
        ...
        mgr = IfLaggManager(mock_client)
        result = await mgr.ensure(state="absent", params={"descr": "Bond0"}, check_mode=True)
...
class TestErrorHandling:
    ...
    async def test_create_failure_logs_error_and_reraises(
        ...
        mock_client.create.side_effect = OpnsenseValidationError(
            message="descr required", endpoint="interfaces/lagg_settings/addItem"
        )
        ...
        with (
            ...
            await mgr.ensure(state="present", params=LAGG_PARAMS)
    ...
    async def test_delete_failure_logs_error_and_reraises(
        ...
        with (
            ...
            await mgr.ensure(state="absent", params={"descr": "Bond0"})
    ...
    async def test_error_preserves_exception_type(self, mock_client: AsyncMock) -> None:
        ...
        mock_client.create.side_effect = OpnsenseValidationError(
            message="bad input",
            endpoint="interfaces/lagg_settings/addItem",
            validations={"lagg.descr": "required"},
        )
        ...
        assert exc_info.value.validations == {"lagg.descr": "required"}
...
class TestFieldValidation:
    ...
    async def test_invalid_proto_raises_before_api_call(self, mock_client: AsyncMock) -> None:
        mgr = IfLaggManager(mock_client)
        with pytest.raises(FieldValidationError):
            await mgr.ensure("present", params={"descr": "Bond0", "proto": "invalid"})
        mock_client.create.assert_not_awaited()
