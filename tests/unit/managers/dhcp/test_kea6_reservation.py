"""Unit tests for opnsense.managers.kea6_reservation.Kea6ReservationManager."""
...
class TestEnsurePresent:
    ...
    async def test_create_new_reservation(self, mock_client: AsyncMock) -> None:
        ...
        mgr = Kea6ReservationManager(mock_client)
...
class TestErrorHandling:
    ...
    async def test_create_failure_logs_error_and_reraises(
        ...
        mock_client.create.side_effect = OpnsenseValidationError(
            message="invalid reservation", endpoint="kea/dhcpv6/addReservation"
        )
...
class TestFieldValidation:
    ...
    async def test_missing_required_ip_address_raises(self, mock_client: AsyncMock) -> None:
        ...
        with pytest.raises(FieldValidationError):
            await mgr.ensure(
                "present",
                params={"duid": "00:03:00:01:00:11:22:33:44:55"},
            )
        mock_client.create.assert_not_awaited()

    async def test_missing_required_duid_raises(self, mock_client: AsyncMock) -> None:
        mgr = Kea6ReservationManager(mock_client)
        with pytest.raises(FieldValidationError):
            await mgr.ensure("present", params={"ip_address": "2001:db8::50"})
        mock_client.create.assert_not_awaited()
...
class TestAmbiguousMatch:
    ...
    async def test_no_ambiguity_different_composite_key(self, mock_client: AsyncMock) -> None:
        """Two reservations with same IP but different DUID -> no ambiguity."""
        mock_client.search.return_value = [
            {
                "uuid": "uuid-1",
                "ip_address": "2001:db8::50",
