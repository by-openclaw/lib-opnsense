"""Unit tests for opnsense.managers.kea4_subnet.Kea4SubnetManager."""
...
class TestEnsurePresent:
    ...
    async def test_create_new_subnet(self, mock_client: AsyncMock) -> None:
        ...
        mgr = Kea4SubnetManager(mock_client)
    ...
    async def test_update_when_drift_detected(self, mock_client: AsyncMock) -> None:
        """ensure present when pools differ -> update + apply."""
        mock_client.search.return_value = [
            {
                "uuid": "uuid-existing",
                "subnet": "10.0.0.0/24",
                "pools": "10.0.0.100-10.0.0.150",
            },
        ]
...
class TestEnsureAbsent:
    ...
    async def test_delete_existing_subnet(self, mock_client: AsyncMock) -> None:
        ...
        mock_client.search.return_value = [
            {"uuid": "uuid-existing", "subnet": "10.0.0.0/24"},
        ]
        mock_client.get.return_value = {
            "subnet4": {"uuid": "uuid-existing", "subnet": "10.0.0.0/24"},
        }
        mock_client.delete.return_value = {"result": "deleted"}
        mock_client.reconfigure.return_value = {"status": "ok"}

        mgr = Kea4SubnetManager(mock_client)
        result = await mgr.ensure(state="absent", params={"subnet": "10.0.0.0/24"})
...
class TestCheckMode:
    ...
    async def test_check_mode_delete_no_api_call(self, mock_client: AsyncMock) -> None:
        mock_client.search.return_value = [
            {"uuid": "uuid-1", "subnet": "10.0.0.0/24"},
        ]
        mock_client.get.return_value = {
            "subnet4": {"uuid": "uuid-1", "subnet": "10.0.0.0/24"},
        }
...
class TestErrorHandling:
    ...
    async def test_create_failure_logs_error_and_reraises(
        ...
        mock_client.create.side_effect = OpnsenseValidationError(
            message="invalid subnet", endpoint="kea/dhcpv4/addSubnet"
        )
    ...
    async def test_delete_failure_logs_error_and_reraises(
        ...
        mock_client.delete.side_effect = OpnsenseValidationError(
            message="delete failed", endpoint="kea/dhcpv4/delSubnet"
        )
        ...
        with (
            ...
            await mgr.ensure(state="absent", params={"subnet": "10.0.0.0/24"})
...
class TestFieldValidation:
    ...
    async def test_missing_required_subnet_raises_before_api_call(
        ...
        with pytest.raises(FieldValidationError):
            await mgr.ensure("present", params={"pools": "10.0.0.100-10.0.0.200"})
        mock_client.create.assert_not_awaited()
