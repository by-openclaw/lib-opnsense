...
"""Integration tests -- Cron job manager against a live OPNsense device.

Requirements:
    - Live OPNsense device accessible via OPN_HOST, OPN_KEY, OPN_SECRET env vars
    - API user must have full admin privileges
    - Run with: pytest tests/integration/services/test_cron_job.py -v

Test flow (ordered, 10-step standard):
    1. Create (C)       -- TestCronJobCRUD.test_01_create
    2. Idempotent (I)   -- TestCronJobCRUD.test_02_idempotent
    3. Update (U)       -- TestCronJobCRUD.test_03_update
    4. Check mode (K)   -- TestCronJobCRUD.test_04_check_mode_create
    ...
    8. Delete (D)       -- TestCronJobCRUD.test_05_delete
...
class TestCronJobCRUD:
    ...
    async def test_04_check_mode_create(self, opn_client: OpnsenseClient) -> None:
        ...
        rows = await mgr.list(search_phrase="inttest-cron-cm")
        found = [row for row in rows if row.get("description") == "inttest-cron-cm"]
        assert found == []
...
class TestAmbiguousMatch:
    """Verify AmbiguousMatchError when >1 cron job matches same description."""

    async def test_01_create_duplicates(self, opn_client: OpnsenseClient) -> None:
        """Create two cron jobs with same description via direct create."""
        mgr = CronJobManager(opn_client)
        dup_params = {
            "description": "inttest-dup-cron",
            "command": "firmware auto-update",
            "minutes": "0",
            "hours": "3",
            "enabled": "0",
    ...
    async def test_03_cleanup_duplicates(self, opn_client: OpnsenseClient) -> None:
        """Delete both duplicate cron jobs by UUID."""
        mgr = CronJobManager(opn_client)
        rows = await mgr.list(search_phrase="inttest-dup-cron")
        for row in rows:
            if row.get("description") == "inttest-dup-cron":
                await mgr.delete(row["uuid"])
        rows = await mgr.list(search_phrase="inttest-dup-cron")
        remaining = [r for r in rows if r.get("description") == "inttest-dup-cron"]
        assert remaining == []
...
class TestFieldValidation:
    ...
    async def test_01_empty_description_rejected(self, opn_client: OpnsenseClient) -> None:
        ...
        with pytest.raises(FieldValidationError, match="description"):
            await mgr.ensure(
                "present",
                {"description": "", "command": "firmware auto-update", "enabled": "0"},
            )
