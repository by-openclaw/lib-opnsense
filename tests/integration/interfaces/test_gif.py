...
"""Integration tests — IfGifManager lifecycle against a live OPNsense device.

Requirements:
    - Live OPNsense device accessible via OPN_HOST, OPN_KEY, OPN_SECRET env vars
    - API user must have full admin privileges
    - Run with: pytest tests/integration/interfaces/test_gif.py -v

Test flow (ordered, 10-step standard):
    1. Create (C)       -- TestGifCRUD.test_01_create
    2. Idempotent (I)   -- TestGifCRUD.test_02_idempotent
    3. Update (U)       -- TestGifCRUD.test_03_update_descr
    ...
    8. Delete (D)       -- TestGifCRUD.test_04_delete
    9. Delete noop (Dn) -- N/A -- no explicit delete-noop test
    10. Cleanup (X)     -- TestCleanup.test_cleanup_gifs
...
class TestGifCRUD:
    """GIF tunnel CRUD — safe with test IPs (10.99.x.x range)."""

    async def test_01_create(self, opn_client: OpnsenseClient) -> None:
        mgr = IfGifManager(opn_client)
        r = await mgr.ensure(
            "present",
            {
                "tunnel-local-addr": "10.99.1.1",
                "tunnel-remote-addr": "10.99.1.2",
                "local-addr": "10.11.1.1",
                "remote-addr": "10.11.1.2",
                "descr": "inttest-gif",
            },
        )
    ...
    async def test_03_update_descr(self, opn_client: OpnsenseClient) -> None:
        """Update descr on existing GIF tunnel -> changed=True, action=updated."""
        mgr = IfGifManager(opn_client)
        # Ensure the GIF tunnel exists first (noop if already present from test_01)
        await mgr.ensure(
            "present",
            {
                "tunnel-local-addr": "10.99.1.1",
    ...
    async def test_04_delete(self, opn_client: OpnsenseClient) -> None:
        ...
        r = await mgr.ensure(
            "absent",
            {"tunnel-local-addr": "10.99.1.1", "tunnel-remote-addr": "10.99.1.2"},
        )
        assert r.changed is True
...
class TestCheckMode:
    ...
    async def test_04_check_mode_create(self, opn_client: OpnsenseClient) -> None:
        ...
        rows = await mgr.list(search_phrase="inttest-gif-checkmode")
        assert not any(r.get("descr") == "inttest-gif-checkmode" for r in rows)


class TestAmbiguousMatch:
    """Prove AmbiguousMatchError fires on duplicate GIF tunnel data.

    Creates two GIF tunnels with identical match keys via direct create(),
    ...
    DUP_PARAMS = {
        ...
        "local-addr": "10.11.1.1",
        "remote-addr": "10.11.1.2",
        "descr": "inttest-gif-dup",
    }

    async def test_05_create_duplicates(self, opn_client: OpnsenseClient) -> None:
        """Create two identical GIF tunnels via direct create (bypass ensure)."""
        mgr = IfGifManager(opn_client)
    ...
    async def test_07_cleanup_duplicates(self, opn_client: OpnsenseClient) -> None:
        """Delete both duplicate GIF tunnels by UUID."""
        mgr = IfGifManager(opn_client)
        rows = await mgr.list(search_phrase="inttest-gif-dup")
        dups = [r for r in rows if r.get("descr") == "inttest-gif-dup"]
        for dup in dups:
            await mgr.delete(dup["uuid"])
        # Verify clean
        rows = await mgr.list(search_phrase="inttest-gif-dup")
        remaining = [r for r in rows if r.get("descr") == "inttest-gif-dup"]
        assert remaining == []
        ...
        with pytest.raises(FieldValidationError):
            await mgr.ensure(
                "present",
                {
                    ...
                    "descr": "inttest-gif-bad",
                },
            )


class TestCleanup:
    """Remove any leftover inttest- GIF tunnel objects."""

    async def test_cleanup_gifs(self, opn_client: OpnsenseClient) -> None:
        mgr = IfGifManager(opn_client)
        rows = await mgr.list(search_phrase="inttest")
        for row in rows:
            if "inttest" in str(row.get("descr", "")):
                await opn_client.delete("interfaces/gif_settings/delItem", row["uuid"])
        await opn_client.reconfigure("interfaces/gif_settings/reconfigure")
