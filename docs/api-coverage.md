...
| acme-accounts | `AcmeAccountManager` | `name` | `acmeclient/accounts` (bare) + `register` | `INTEGRATION_TEST_PASSED` | CA enum (`letsencrypt`/`letsencrypt_test`/buypass/google/sslcom/zerossl/custom). Custom verb `register(uuid)` registers with the CA. Redacts key/eab_hmac/eab_kid. |
...
| acme-certificates | `AcmeCertificateManager` | `name` | `acmeclient/certificates` (bare) + `sign`/`revoke`/`removekey`/`automation`/`import` | `INTEGRATION_TEST_PASSED` | References `account`+`validationMethod`+`restartActions` (UUIDs; option-dicts on read, DiffEngine normalises). `sign(uuid)` issues; issued leaf lands in trust store under `certRefId`. keyLength enum. |
